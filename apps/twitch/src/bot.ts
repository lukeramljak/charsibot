...
export class Bot {
        ...
        e => {
          ...
          switch (cmd) {
            ...
            case '!rmstat':
              ...
              break;
            case '!coobubu':
              this.handleShowCollectionCommand('coobubu', e.chatterId, e.chatterDisplayName);
              break;
            case '!coobubu-redeem':
              this.handleRedeemBlindBoxCommand('coobubu', e.chatterId, e.chatterDisplayName);
              break;
            case '!olliepop':
              this.handleShowCollectionCommand('olliepops', e.chatterId, e.chatterDisplayName);
              break;
            case '!olliepop-redeem':
              this.handleRedeemBlindBoxCommand('olliepops', e.chatterId, e.chatterDisplayName);
              break;
          }
        }
      );

      this.listener.onChannelRedemptionAdd(this.config.channelUserId, async e => {
        log.info(
        ...
        if (e.rewardTitle === 'Cooper Series Blind Box') {
          await this.handleRedeemBlindBoxCommand('coobubu', e.userId, e.userName);
        } else if (e.rewardTitle === 'Ollie Series Blind Box') {
          await this.handleRedeemBlindBoxCommand('olliepops', e.userId, e.userName);
        }
  ...
  private async handleRedeemBlindBoxCommand(
    ...
    const result = await this.store.addPlushieToCollection(userId, username, type, plushieKey);
